/**
 * sprites.js
 * Pixel-perfect Minecraft sprite renderer.
 * Each sprite is defined as a 16-column hex-color array (row by row).
 * 0 = transparent. All colors taken directly from Minecraft's official textures.
 */

'use strict';

/* ─────────────────────────────────────────────────────────────
   COLOR PALETTE  (official Minecraft texture hex values)
───────────────────────────────────────────────────────────── */
const T = 0;          // transparent
const _ = 0xFFFFFFFF; // alias
...
/* ─────────────────────────────────────────────────────────────
   SPRITE DEFINITIONS  (16×16 pixel maps, each row = 16 entries)
───────────────────────────────────────────────────────────── */

const SPRITES = {};

/* ── GRASS BLOCK (top face + front face, 16×16) ── */
SPRITES.grass = {
  w:16, h:16,
  rows:[
...
SPRITES.flower = {
  ...
  rows:[
    ...
    [T,T,T,T,T,FL.r1,FL.r2,FL.r3,FL.r1,FL.r2,T,T,T,T,T,T],
    [T,T,T,T,T,FL.r2,FL.r1,FL.r3,FL.r2,FL.r1,T,T,T,T,T,T],
    [T,T,T,T,T,T,FL.r3,FL.r2,FL.r3,T,T,T,T,T,T,T],
    [T,T,T,T,T,T,T,FL.g1,T,T,T,T,T,T,T,T],
    [T,T,T,T,T,T,FL.g2,FL.g1,T,T,T,T,T,T,T,T],
    [T,T,T,T,T,T,T,FL.g3,T,FL.g1,T,T,T,T,T,T],
    [T,T,T,T,T,T,T,FL.g1,FL.g2,T,T,T,T,T,T,T],
...
/* ─────────────────────────────────────────────────────────────
   MOB SPRITES  (multi-frame for walk animation)
   Each mob has frames[] — array of 2D color arrays (no scale yet)
───────────────────────────────────────────────────────────── */

/* Helper: build a Minecraft-style character frame
   We define per-mob frame builders that return a 2D RGBA array */

// ── STEVE (24×32 native MC proportions, scaled from 8px units) ──
// We use a simplified 16×24 view rendered at 3× scale
function steveFrame(legPhase) {
  // legPhase: 0=neutral,1=left-forward,2=right-forward
  const S = SS;
  // 16 wide × 24 tall
  const rows = [];

  // HEAD (8 rows)
  rows.push([T,T,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h1,S.h1,S.h1,S.h1,S.h1,S.h1,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h1,S.h3,S.h1,S.h1,S.h3,S.h1,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h1,S.h1,S.h1,S.h1,S.h1,S.h1,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h1,S.h1,S.h5,S.h5,S.h6,S.h1,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h1,S.h5,S.h5,S.h6,S.h5,S.h1,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h2,S.h2,S.h2,S.h2,S.h2,S.h2,S.h4,T,T,T,T,T,T]);
  rows.push([T,T,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,S.h4,T,T,T,T,T,T]);

  // BODY (4 rows)
  rows.push([T,S.h1,S.b3,S.b1,S.b1,S.b1,S.b1,S.b1,S.b1,S.b3,S.h1,T,T,T,T,T]);
  rows.push([T,S.h2,S.b1,S.b2,S.b1,S.b1,S.b2,S.b1,S.b2,S.b1,S.h2,T,T,T,T,T]);
  rows.push([T,S.h1,S.b2,S.b1,S.b4,S.b1,S.b1,S.b4,S.b1,S.b2,S.h1,T,T,T,T,T]);
  rows.push([T,S.h2,S.b1,S.b2,S.b1,S.b1,S.b2,S.b1,S.b2,S.b1,S.h2,T,T,T,T,T]);

  // LEGS (variable per frame, 8 rows)
  if (legPhase === 0) {
    // neutral — legs together
    for (let r=0;r<8;r++){
      if(r<4) rows.push([T,T,S.p1,S.p1,S.p2,T,T,S.p1,S.p1,S.p2,T,T,T,T,T,T]);
      else    rows.push([T,T,S.s1,S.s1,S.s2,T,T,S.s1,S.s1,S.s2,T,T,T,T,T,T]);
    }
  } else if (legPhase === 1) {
    // left leg forward
    rows.push([T,T,T,S.p1,S.p2,S.p1,T,S.p1,S.p1,S.p2,T,T,T,T,T,T]);
    rows.push([T,T,T,S.p2,S.p1,S.p2,T,S.p2,S.p1,S.p2,T,T,T,T,T,T]);
    rows.push([T,T,T,S.p1,S.p2,S.p1,T,S.p1,S.p2,S.p1,T,T,T,T,T,T]);
    rows.push([T,T,T,S.p3,S.p1,S.p3,T,S.p3,S.p1,S.p3,T,T,T,T,T,T]);
    rows.push([T,T,T,T,S.s1,S.s2,T,T,S.s1,S.s2,T,T,T,T,T,T]);
    rows.push([T,T,T,T,S.s2,S.s1,T,T,S.s2,S.s1,T,T,T,T,T,T]);
    rows.push([T,T,T,S.s1,S.s2,T,T,T,S.s1,S.s2,T,T,T,T,T,T]);
    rows.push([T,T,S.s2,S.s1,T,T,T,T,S.s2,S.s1,T,T,T,T,T,T]);
  } else {
    // right leg forward
    rows.push([T,T,S.p1,S.p1,S.p2,T,T,T,S.p1,S.p2,S.p1,T,T,T,T,T]);
    rows.push([T,T,S.p2,S.p1,S.p2,T,T,T,S.p2,S.p1,S.p2,T,T,T,T,T]);
    rows.push([T,T,S.p1,S.p2,S.p1,T,T,T,S.p1,S.p2,S.p1,T,T,T,T,T]);
    rows.push([T,T,S.p3,S.p1,S.p3,T,T,T,S.p3,S.p1,S.p3,T,T,T,T,T]);
    rows.push([T,T,S.s1,S.s2,T,T,T,T,T,S.s1,S.s2,T,T,T,T,T]);
    rows.push([T,T,S.s2,S.s1,T,T,T,T,T,S.s2,S.s1,T,T,T,T,T]);
    rows.push([T,T,S.s1,S.s2,T,T,T,T,S.s1,S.s2,T,T,T,T,T,T]);
    rows.push([T,T,S.s2,S.s1,T,T,T,T,S.s2,S.s1,T,T,T,T,T,T]);
  }
...
function creeperFrame(legPhase) {
  ...
  if (legPhase===0){
    ...
    const off=legPhase===1?1:-1;
    for(let r=0;r<12;r++){
      const s=Math.min(2,Math.floor(r/3));
      rows.push([T,C[lc[r%4]],C.g1,T,T,T,T,C[lc[(r+2)%4]],C.g2,T,T,T]);
    }
  }
  return { w:12, h:24, rows };
}
...
/* ─────────────────────────────────────────────────────────────
   RENDERER  —  draw a sprite object onto a canvas
───────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────
   ANIMATED MOB  — creates a canvas + runs walk cycle
───────────────────────────────────────────────────────────── */
function createMob(type, scale, speed, startX, dir, layer) {
  ...
  let frame = 0;
  let x = startX;
  const facing = dir > 0 ? 1 : -1; // 1=right, -1=left
  ...
  // Walk animation
  let lastT = 0;
  const frameMs  = 220;  // ms per walk frame
  const pixelsMs = speed; // px per ms
...
/* Export everything for script.js */
window.MC = {
